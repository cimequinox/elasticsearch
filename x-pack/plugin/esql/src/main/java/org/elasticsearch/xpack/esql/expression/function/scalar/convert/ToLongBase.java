...
public class ToLongBase extends EsqlScalarFunction {
    ...
    // ──────────────────────────────────────────────────────── Pow
    public ToLongBase(Source source, Expression field, Expression base) {
        super(source, List.of(field, base));
        this.field = field;
        this.base = base;
    }
    ...
    public boolean foldable() {
        return field.foldable() && base.foldable();
    }

    // ──────────────────────────────────────────────────────── ToLong, Pow
    @Evaluator
    static long process(BytesRef in, int radix) {
        var value = in.utf8ToString();
        try {
            if (radix == 10) {
    ...
    protected NodeInfo<? extends Expression> info() {
        return NodeInfo.create(this, ToLongBase::new, field, base);
    }
    ...
    public ExpressionEvaluator.Factory toEvaluator(ToEvaluator toEvaluator) {
        var stringEval = toEvaluator.apply(field);
